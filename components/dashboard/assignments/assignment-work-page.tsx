...
export function AssignmentWorkPage({
  ...
  const { data: mySubmission, isFetching: isFetchingMySubmission } = useQuery({
  ...
  const pageSize =
    assignment?.type === "QUIZ_ESSAY" ? essayPageSize : mcqPageSize;
  ...
  const workspaceWrapperClass =
    isDraft || isClosed
      ? "relative max-h-[70vh] min-h-[360px] overflow-hidden rounded-xl"
      : "relative";

  const answeredMcqCount = Object.values(mcqAnswers).filter(Boolean).length;
  const answeredEssayCount = Object.values(essayAnswers).filter((value) =>
    value.trim(),
  ).length;
  ...
  if (!isStudent) {
    return (
      <Card>
        <CardContent className="py-8 text-sm text-muted-foreground">
          This workspace is only for students. Use assignment detail page for
          management view.
        </CardContent>
      </Card>
    );
  ...
  return (
    <section className="space-y-6">
        ...
        <Card className="border-emerald-300/60 bg-emerald-50/50">
          <CardContent className="flex items-start gap-2 py-4 text-sm text-emerald-900">
            ...
            <div>
              ...
              <p>
                Submitted at {formatDateTimeLabel(mySubmission?.submittedAt)}.
                You can update and resubmit before assignment closes.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className={workspaceWrapperClass}>
        <div
          className={isDraft ? "pointer-events-none select-none blur-md" : ""}
        >
          {assignment.type === "QUIZ_MCQ" ? (
            <Card className="border-border/70">
              <CardHeader>
                ...
                <CardDescription>
                  Answered {answeredMcqCount} of {normalizedContent.mcq.length}{" "}
                  questions.
                </CardDescription>
              </CardHeader>
          ...
          {assignment.type === "QUIZ_ESSAY" ? (
            <Card className="border-border/70">
              <CardHeader>
                ...
                <CardDescription>
                  Answered {answeredEssayCount} of{" "}
                  {normalizedContent.essay.length} questions. Showing{" "}
                  {essayPageSize} questions per page.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
          ...
          {assignment.type === "TASK" || assignment.type === "REMEDIAL" ? (
            <Card className="border-border/70">
              <CardHeader>
                ...
                <CardDescription>
                  Write your submission text below.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Textarea
          ...
          <div className="flex justify-end gap-2 border-t border-border/60 pt-4">
            ...
            <Button
              type="button"
              disabled={
                submitMutation.isPending || !canSubmit || isFetchingMySubmission
              }
              onClick={() => {
                if (assignment.type === "QUIZ_MCQ") {
                    ...
                    answers: {
                      ...
                      responses: normalizedContent.mcq
                        ...
                        .filter(
                          (
                            row,
                          ): row is { questionId: string; answer: McqOption } =>
                            row.answer === "A" ||
                            row.answer === "B" ||
                            row.answer === "C" ||
                            row.answer === "D",
        ...
        {isDraft || isClosed ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-background/60 p-4 backdrop-blur-sm">
            <Empty className="max-w-md">
              <EmptyHeader>
                ...
                <EmptyTitle>
                  Assignment is {isDraft ? "not published yet" : "closed"}
                </EmptyTitle>

                <EmptyDescription>
                  {isDraft
                    ? "Students can only see and work on assignment content after it is published by teacher."
                    : "The assignment is closed. You cannot submit any answers."}
                </EmptyDescription>
              </EmptyHeader>
              ...
              <EmptyContent>
                <p className="text-xs text-muted-foreground text-center">
                  ...
                  {isDraft ? (
                    <>
                      and wait until status changes to{" "}
