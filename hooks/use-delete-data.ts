...
type UseDeleteDataParams<TData, TVariables, TContext> = {
  key?: MutationKey;
...
export function useDeleteData<TData, TVariables = unknown, TContext = unknown>({
  key,
  endpoint,
  params,
  headers,
  ...
  extractData = true,
  options,
}: UseDeleteDataParams<TData, TVariables, TContext>) {
  ...
  return useMutation<TData, AxiosError, TVariables, TContext>({
    ...
    mutationFn: async (variables) => {
      ...
      const response = await axiosInstance.delete<ApiEnvelope<TData> | TData>(resolvedEndpoint, {
