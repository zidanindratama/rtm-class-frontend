...
type UsePatchDataParams<TData, TVariables, TContext> = {
  key?: MutationKey;
...
export function usePatchData<TData, TVariables = unknown, TContext = unknown>({
  key,
  endpoint,
  params,
  headers,
  ...
  extractData = true,
  options,
}: UsePatchDataParams<TData, TVariables, TContext>) {
  ...
  return useMutation<TData, AxiosError, TVariables, TContext>({
    ...
    mutationFn: async (variables) => {
      ...
      const response = await axiosInstance.patch<ApiEnvelope<TData> | TData>(
        resolvedEndpoint,
        variables,
        {
