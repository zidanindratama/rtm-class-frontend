...
type UsePutDataParams<TData, TVariables, TContext> = {
  key?: MutationKey;
...
export function usePutData<TData, TVariables = unknown, TContext = unknown>({
  key,
  endpoint,
  params,
  headers,
  ...
  extractData = true,
  options,
}: UsePutDataParams<TData, TVariables, TContext>) {
  ...
  return useMutation<TData, AxiosError, TVariables, TContext>({
    ...
    mutationFn: async (variables) => {
      ...
      const response = await axiosInstance.put<ApiEnvelope<TData> | TData>(
        resolvedEndpoint,
        variables,
        {
